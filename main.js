...
let
    ...
    estilos = e => {
        css('celda', 'width', (100 / (e.numeros || 75)) + '%')
        css('celda>svg', 'max-height', (50 / (e.filas || 5)) + 'vh')
    ...
    desmarcar = num => {
        ...
        celda.getElementsByTagName('text')[0].removeAttribute('fill')
    },
    marcar = num => {
        let celda = document.getElementById('c_' + num)
        if (celda == undefined) return
        ...
        celda.getElementsByTagName('text')[0].setAttribute('fill', 'white')
    ...
    getTecla = e => {
        ocultarEjemplo()
        e.keyCode == 32 ? sortear() : null
            //e.keyCode == 72 ? mostrarEjemplo("h") : null
            //e.keyCode == 79 ? mostrarEjemplo("o") : null
            //e.keyCode == 67 ? mostrarEjemplo("c") : null
            //e.keyCode == 88 ? mostrarEjemplo("x") : null
            //e.keyCode == 78 ? mostrarEjemplo("n") : null
            //e.keyCode == 49 ? mostrarEjemplo("vertical") : null
            //e.keyCode == 50 ? mostrarEjemplo("horizontal") : null
    },
    mostrarEjemplo = v => {
        let __e = document.querySelector('#ejemplo')
        __e.classList = ["mostrarEjemplo"]
        __e.style.backgroundImage = "url('img/" + v + ".png')"
    },
    ocultarEjemplo = () => {
        let __e = document.querySelector('#ejemplo')
        __e.classList = ["ocultarEjemplo"]
    }